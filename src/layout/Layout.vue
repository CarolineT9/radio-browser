<template>
  <div class="flex flex-col w-full">
    ...
    >
      ...
      >
        <div class="flex justify-between items-center mb-8">
          <span class="text-bg text-2xl font-semibold tracking-widest"
            >Menu</span
          >
          <button
    ...
    >
      ...
      <div
        class="max-[820px]:hidden w-full flex items-center justify-between gap-8"
      >
        <DesktopNavGroup
          :sections="desktopNav"
          :separators="true"
          :dropdownSlotSubtitle="'by Name'"
          @input-submit="onDesktopSubmit"
        >
          <template #dropdown="{ section, index, submit }">
            <div
              class="flex flex-col gap-2 h-[200px] align-center overflow-auto"
            >
              <Input :placeholder="'Search stations…'" :type="'text'" />
              <ul
                v-if="section.links"
                class="flex text-secondary flex-col gap-2"
              >
    ...
    <div class="flex-1 w-full flex items-center justify-center gap-16 max-[1080px]:gap-8">
     <Card :class="'w-[400px] max-[1080px]:w-[min(92vw,420px)] h-[500px] '"/>
     <Card :class="'w-[400px] max-[1080px]:w-[min(92vw,420px)] h-[550px]'"/>
    </div>
  </div>
</template>
<script setup>
